...
let activeCategory = document.querySelector('.category-option.active');
    ...
    option.addEventListener('click', function() {
        ...
        if (this.dataset.category === 'custom') {
            ...
            if (customCategory && customCategory.trim() !== '') {
                ...
                activeCategory = document.querySelector('.category-option[data-category="monthly"]');
...
document.querySelector('.add-btn').addEventListener('click', function() {
    ...
    const recurring = document.getElementById('recurring-income').checked ? 1 : 0;
    ...
    const sql = `INSERT INTO transactions (user_id, type, name, amount, category, date, notes, recurring)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?)`;
    const params = [userId, 'income', name, amount, category, date, notes, recurring];
    window.AndroidDB.execute(sql, JSON.stringify(params));

    showSuccessAndRedirect('Income added successfully!');
});
